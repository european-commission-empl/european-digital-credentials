...
export class DiplomaToolbarComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        this.credId = this.shareDataService.credUUID;
        this.userId = this.shareDataService.walletAddress;
        this.sharedFromURL = this.shareDataService.shareLink;
    ...
    readFile(event: FileList): void {
        if (event[0].type.endsWith('/xml')) {
            ...
            reader.onloadend = () => {
                ...
                this.onUploadNewCredential.emit();
            };
        }
    ...
    private resetVerificationSteps(): void {
        this.shareDataService.verificationChecks = null;
    }
