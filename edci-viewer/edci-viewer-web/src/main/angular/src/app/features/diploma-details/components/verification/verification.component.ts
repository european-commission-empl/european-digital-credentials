...
import { IDiplomaDetailsResolverData } from '@shared/model/IDiplomaDetailsResolverData';
...
export class VerificationComponent implements OnInit, OnDestroy {
    ...
    isPreview: boolean;
    steps: VerificationCheckView[] = this.shareDataService.verificationChecks;
    ...
    ngOnInit() {
        ...
        this.isPreview = this.shareDataService.isPreview;
        this.language = this.shareDataService.toolbarLanguage;
        this.shareDataService.getVerificationStepsObs().pipe(takeUntil(this.destroy$)).subscribe({
            next: (verificationSteps) => {
                this.saveStepsShareDataService(verificationSteps);
            }
        });
        ...
        this.translateService
            ...
            .subscribe((translations) => {
                ...
                this.ribbonLabel.warning =
                    ...
                    'Partially Verified';
                this.steps[this.steps.length - 1].type.title[0] =
                    this.ribbonState === 2
                        ? this.ribbonLabel.incorrect
                        : this.ribbonState === 1
                            ? this.ribbonLabel.warning
    ...
    private saveStepsShareDataService(steps: VerificationCheckView[]) {
        if (steps) {
            this.shareDataService.verificationChecks = steps;
            this.shareDataService.setVerificationSteps(steps);
            this.displaySteps(steps);
        }
    }

    private displaySteps(steps: VerificationCheckView[]) {
        if (steps) {
