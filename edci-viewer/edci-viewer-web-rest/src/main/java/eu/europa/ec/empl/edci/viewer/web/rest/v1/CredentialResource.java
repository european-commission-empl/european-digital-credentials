...
public class CredentialResource implements CrudResource {
    ...
    public EuropassCredentialPresentationView getCredentialDetail(@RequestPart(value = "file") MultipartFile file, @RequestParam(value = Parameter.LOCALE) String locale) {
        ...
        CredentialHolderDTO credential = credentialDetailService.getCredentialDetail(file);
    ...
    public EuropassCredentialPresentationView getCredentialDetail(@PathVariable(Parameter.WALLET_USER_ID) String userId, @PathVariable(Parameter.UUID) String credentialUUID,
        ...
        //We are retrieving the credential's primary language if no lang is passed. This is done because when opening a credential from the viewer, we don't know which lang it has
        CredentialHolderDTO credential = credentialDetailService.getCredentialDetail(userId, credentialUUID);
        if (StringUtils.isBlank(locale)) {
            LocaleContextHolder.setLocale(LocaleUtils.toLocale(credential.getCredential().getPrimaryLanguage()));
        }
