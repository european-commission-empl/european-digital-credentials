...
//    private final static Logger logger = LoggerFactory.getLogger(CredentialDetailService.class);
    ...
    public EuropassDiplomaDTO getWalletDiplomaHTML(String credentialUUID, String walletUserId, String locale) {
        ...
        try {
            response = walletResourceUtils.doWalletGetRequest(viewerConfigService.getString(Constants.CONFIG_PROPERTY_WALLET_DOWNLOAD_XML).replaceAll(Pattern.quote(Parameter.WALLET_USER_ID),
                    walletUserId).replaceAll(Pattern.quote(Parameter.UUID), credentialUUID).concat("?retrieveVP=true"), null, MediaType.APPLICATION_OCTET_STREAM, String.class, false);
        ...
        return europassCredentialViewerUtils.extractEuropassDiplomaDTO(europassCredentialDTO.getCredential(), locale);
    }
    ...
    public EuropassDiplomaDTO getCredentialDiploma(MultipartFile file, String locale) {
        ...
        try {
           /* jaxbContext = JAXBContext.newInstance(EuropassCredentialDTO.class);
            ...
            europassCredentialDTO = edciCredentialModelUtil.fromByteArray(file.getBytes()).getCredential();
    ...
    public List<VerificationCheckView> getWalletCredentialVerification(String credentialUUID, String walletUserId) {
        ...
        try {
            verificationCheckViews = Arrays.asList(walletResourceUtils.doWalletGetRequest(
                    viewerConfigService.getString(Constants.CONFIG_PROPERTY_WALLET_VERIFY_ID_URL)
    ...
    public List<VerificationCheckView> getCredentialVerification(MultipartFile file) {
        List<VerificationCheckView> verificationCheckViews = null;
        verificationCheckViews = Arrays.asList(walletResourceUtils.doWalletPostRequest(viewerConfigService.getString(Constants.CONFIG_PROPERTY_WALLET_VERIFY_XML_URL), file, EDCIParameter.WALLET_CREDENTIAL_FILE, VerificationCheckView[].class, MediaType.APPLICATION_JSON, false));
        return verificationCheckViews;
    }
