...
public class EDCIOauth2TokenService {
    ...
    public void reloadFromEuTokenExchange(String audience) {
        if (this.getEdciAuthenticationService().isAuthenticated()) {
            ...
            String sessionId = RequestContextHolder.currentRequestAttributes().getSessionId();
            logger.debug(String.format("OIDC - Requesting access token via EU Login Token Exchange for user %s and session id %s with RefreshToken %s...", this.getEdciUserService().getUserId(), sessionId, this.getEdciAuthenticationService().getRefreshToken().substring(0, 10)));
            EDCITokenEndpointResponse tokenResponse = new EDCIRestRequestBuilder(HttpMethod.POST, tokenEndPoint)
                    .addHeaderRequestedWith()
                    .addHeaders(headers)
                    .addBody(parameters)
    ...
    public void reloadFromKeycloakTokenExchange(String audience) {
        if (this.getEdciAuthenticationService().isAuthenticated()) {
            ...
            String sessionId = RequestContextHolder.currentRequestAttributes().getSessionId();
            logger.debug(String.format("OIDC - Requesting access token via Keycloak Token Exchange for user %s and session id %s with RefreshToken %s...", this.getEdciUserService().getUserId(), sessionId, this.getEdciAuthenticationService().getRefreshToken().substring(0, 10)));
            EDCITokenEndpointResponse tokenResponse = new EDCIRestRequestBuilder(HttpMethod.POST, tokenEndPoint)
                    .addHeaderRequestedWith()
                    .addHeaders(headers)
                    .addBody(parameters)
