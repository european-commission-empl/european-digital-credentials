...
public class EDCILabelImporterUtil {
    ...
    private static final String CONVERISON_DIRECTORY = "C:/Dev/EDCI/git/edci_conversion";
    private static final String ESEAL_DIRECTORY = "C:/Dev/EDCI/git/edci_eseal";
    private static final String VERIFICATION_DIRECTORY = "C:/Dev/EDCI/git/edci_verification";
    private static final String EDCI_DIRECTORY = "C:/Dev/EDCI/git/edci";
...
//    private static final List<String> LANGUAGES = Arrays.asList("en");
    ...
    public static void main(String[] args) {
        ...
        System.out.println(">>>>>>>>>>>>>>>>> Source file name (no path is needed): " + args[0]); //Labels_1.3.xlsx
        ...
        EDCILabelImporterUtil.updateAndAppendBackendLabels("conversion", "Conversion Back", sourceFileName);
        EDCILabelImporterUtil.updateAndAppendBackendLabels("eseal", "Eseal Back", sourceFileName);
        EDCILabelImporterUtil.updateAndAppendBackendLabels("verification", "Verification Back", sourceFileName);
...
//                value = value.replace("”","\"");
            ...
            for (Map.Entry<String,String> entry : set) {
                String value = entry.getValue();
                value = value.replaceAll("(')(')?(\\w|\\s)","''$3"); //Fix simple quotes
                updatedProperty.put(entry.getKey(), value);
            }
    ...
    private static File readBackLabelsFile(String appName, String language) {
        String rootfile = EDCI_DIRECTORY;

        if ("conversion".equals(appName)) {
            rootfile = CONVERISON_DIRECTORY;
        } else if ("eseal".equals(appName)) {
            rootfile = ESEAL_DIRECTORY;
        } else if ("verification".equals(appName)) {
            rootfile = VERIFICATION_DIRECTORY;
        }
    ...
    private static Map<String, String> getLangLiteralsMap(Sheet sheet, String lang) {
        ...
        for (Row rows : sheet) {
            if (rows.getRowNum() == 0) {
                ...
                Cell keysRow = rows.getCell(keyCell);
                Cell langLiteralRow = rows.getCell(langCell);
                try {
