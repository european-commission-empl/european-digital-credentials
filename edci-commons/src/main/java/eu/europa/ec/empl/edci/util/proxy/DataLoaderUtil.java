...
public class DataLoaderUtil {
    ...
    public static final Logger logger = Logger.getLogger(DataLoaderUtil.class);
    ...
    public void downloadAsset(DownloadableAsset downloadableAsset, Function<String, Code> findCodeByTargetName, ImageUtil imageUtil, Code encoding, Annotation annotation) throws IOException {
        if (downloadableAsset instanceof DownloadableEntity) {
            DownloadableEntity downloadableEntity = (DownloadableEntity) downloadableAsset;
            downloadEntity(downloadableEntity, findCodeByTargetName, imageUtil, encoding, annotation);
        } else {
            downloadAsset(downloadableAsset);
        }
    }

    public void downloadAsset(DownloadableAsset downloadableAsset) throws IOException {
        DownloadableAsset downloadedAsset = self.doGetDownloadableObject(downloadableAsset.getContentUrl());
        downloadableAsset.setContentUrl(downloadedAsset.getContentUrl());
        downloadableAsset.setContent(downloadedAsset.getContent());
    }

    @Cacheable("DA_DownloadableObject")
    public DownloadableAsset doGetDownloadableObject(URI contentUrl) throws IOException {
        ...
        try {
            downloadableObject.setContentUrl(contentUrl);
            httpRequest = new HttpGet(contentUrl);
            if (this.contentType != null) {
                httpRequest.setHeader("Content-Type", this.contentType);
            }

            client = this.getHttpClient(downloadableObject.getContentUrl().toString());
            httpResponse = this.getHttpResponse(client, httpRequest);

            HttpEntity responseEntity = httpResponse.getEntity();
            if (httpResponse.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
                inputStream = responseEntity.getContent();
                downloadableObject.setContent(IOUtils.toByteArray(inputStream));
    ...
    @Cacheable(value = "DE_MediaObject", key = "#contentUrl.toString()")
    public DownloadableEntity doGetMediaObject(URI contentUrl, Function<String, Code> findCodeByTargetName, ImageUtil imageUtil, Code encoding, Annotation annotation) throws IOException {
        ...
        try {
            ...
            client = this.getHttpClient(mediaObject.getContentUrl().toString());
            httpResponse = this.getHttpResponse(client, httpRequest);
            logger.debug("downloading resource " + contentUrl.toString());
            HttpEntity responseEntity = httpResponse.getEntity();
            if (httpResponse.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
                inputStream = responseEntity.getContent();

                String headerContent = httpResponse.getFirstHeader(HttpHeaders.CONTENT_TYPE).getValue();

                if ("image".equalsIgnoreCase(headerContent.split("/")[0])) {
                    if (annotation != null && annotation instanceof Resize) {
                        Resize annotResize = (Resize) annotation;
                        mediaObject.setContent(imageUtil.resizeImage(IOUtils.toByteArray(inputStream), headerContent.split("/")[1], annotResize.height(), annotResize.width()));
                    } else {
                        mediaObject.setContent(IOUtils.toByteArray(inputStream));
                    }
                } else {
                    mediaObject.setContent(IOUtils.toByteArray(inputStream));
                }
    ...
    public void downloadEntity(DownloadableEntity downloadableEntity, Function<String, Code> findCodeByTargetName, ImageUtil imageUtil, Code encoding, Annotation annotation) throws IOException {
        DownloadableEntity mediaObject = self.doGetMediaObject(downloadableEntity.getContentUrl(), findCodeByTargetName, imageUtil, encoding, annotation);
        downloadableEntity.setContentEncoding(mediaObject.getContentEncoding());
        downloadableEntity.setContent(mediaObject.getContent());
        downloadableEntity.setContentType(mediaObject.getContentType());
        downloadableEntity.setContentUrl(mediaObject.getContentUrl());
    }
