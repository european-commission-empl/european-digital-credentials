    ...
    default EuropassCredentialPresentationView toEuropassCredentialPresentationView(CredentialHolderDTO credentialHolderDTO, @Context Boolean addLists) {
        ...
        if (credentialHolderDTO != null && credentialHolderDTO instanceof EuropassPresentationDTO) {
            ...
            if (credentialHolderDTO.getType() != null && credentialHolderDTO.getType().getTargetName() != null) {
                view.getCredentialMetadata().setVpType(credentialHolderDTO.getType().getTargetName().getStringContent(LocaleContextHolder.getLocale().getLanguage()));
            }

            view.getCredentialMetadata().setFormatType(credentialHolderDTO instanceof EuropassPresentationDTO ? EDCIConstants.Certificate.CREDENTIAL_TYPE_EUROPASS_PRESENTATION : EDCIConstants.Certificate.CREDENTIAL_TYPE_EUROPASS_CREDENTIAL);
...
//    }
    ...
    List<OrganizationTabView> toOrganizationTabViewList(List<OrganizationDTO> organizationDTOS);
    ...
    @Mappings({
            ...
            @Mapping(source = "startedAtTime", target = "startDate", qualifiedByName = "toStringDateLocal"),
            @Mapping(source = "endedAtTime", target = "endDate", qualifiedByName = "toStringDateLocal")
    })
    ...
    @Mappings({
            @Mapping(source = "wasAwardedBy.awardingDate", target = "awardingDate", qualifiedByName = "toStringDateLocal"),
    ...
    @Mappings({
            @Mapping(target = "issueDate", qualifiedByName = "toStringDateLocal"),
            @Mapping(target = "reviewDate", qualifiedByName = "toStringDateLocal"),
            @Mapping(target = "expiryDate", qualifiedByName = "toStringDateLocal")
    })
    AccreditationFieldView toFieldAccreditation(AccreditationDTO accr);

    default String qualificationToString(QualificationDTO accr) {

        String returnValue = null;

        if (accr != null) {
            Text text = accr.getTitle();
            if (text != null) {
                returnValue = text.getStringContent();
            }
        }
