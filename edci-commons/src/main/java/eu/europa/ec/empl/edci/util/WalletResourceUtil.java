...
public class WalletResourceUtil implements InitializingBean {
    ...
    public static final Logger logger = LogManager.getLogger(WalletResourceUtil.class);
    ...
    public <T> T doWalletGetRequest(String url, MediaType contentType, MediaType accept, Class<T> responseType, boolean addAuthToken, Map<String, String> parameters) {
    ...
    public <T> T doWalletPostRequest(String url, String body, Class<T> responseType, MediaType contentType, MediaType accept, boolean addAuthToken, Map<String, String> parameters) {
    ...
    public <T> ResponseEntity<Resource<T>> doWalletPostRequest(String url, String body, ParameterizedTypeReference<Resource<T>> responseType, MediaType contentType, MediaType accept, boolean addAuthToken, Map<String, String> parameters) {
        EDCIRestRequestBuilder edciRestRequestBuilder = new EDCIRestRequestBuilder(HttpMethod.POST, url);
        if (addAuthToken) {
            this.getOauth2TokenService().reloadAccessToken(walletClientId);
            edciRestRequestBuilder.addAuthenticationToken(this.getEdciAuthenticationService().getAccessToken());
    ...
    public <T> T doWalletPostRequest(String url, MultipartFile body, String paramName, Class<T> responseType, MediaType accept, boolean addAuthToken, Map<String, String> parameters) {
        EDCIRestRequestBuilder edciRestRequestBuilder = new EDCIRestRequestBuilder(HttpMethod.POST, url);
        if (addAuthToken) {
            this.getOauth2TokenService().reloadAccessToken(walletClientId);
            edciRestRequestBuilder.addAuthenticationToken(this.getEdciAuthenticationService().getAccessToken());
