...
public class WalletResourceUtil implements InitializingBean {
    ...
    public <T> T doWalletGetRequest(String url, MediaType contentType, MediaType accept, Class<T> responseType, boolean addAuthToken) throws OIDCException {
    ...
    public <T> T doWalletPostRequest(String url, String body, Class<T> responseType, MediaType contentType, MediaType accept, boolean addAuthToken) throws OIDCException {
    ...
    public <T> ResponseEntity<Resource<T>> doWalletPostRequest(String url, String body, ParameterizedTypeReference<Resource<T>> responseType, MediaType contentType, MediaType accept, boolean addAuthToken) throws OIDCException {
        EDCIRestRequestBuilder edciRestRequestBuilder = new EDCIRestRequestBuilder(HttpMethod.POST, url);
        if (addAuthToken) {
            this.getOauth2TokenService().reloadAccessToken(walletClientId);
            edciRestRequestBuilder.addAuthenticationToken(this.getEdciAuthenticationService().getAccessToken());
    ...
    public <T> T doWalletPostRequest(String url, MultipartFile body, String paramName, Class<T> responseType, MediaType accept, boolean addAuthToken) throws OIDCException {
        EDCIRestRequestBuilder edciRestRequestBuilder = new EDCIRestRequestBuilder(HttpMethod.POST, url);
        if (addAuthToken) {
            this.getOauth2TokenService().reloadAccessToken(walletClientId);
            edciRestRequestBuilder.addAuthenticationToken(this.getEdciAuthenticationService().getAccessToken());
