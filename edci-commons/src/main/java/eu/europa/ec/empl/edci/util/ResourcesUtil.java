...
public class ResourcesUtil {
    ...
    public static final Logger logger = Logger.getLogger(ResourcesUtil.class);
    ...
    public <S extends Identifiable> void checkContentClassLoopTree(S elem) {
        checkContentClassLoopTree(new LinkedHashMap<Integer, String>(), elem);
    }

    protected <S extends Identifiable> void checkContentClassLoopTree(LinkedHashMap<Integer, String> orgPkList, S elem) {

        if (elem == null) {
            return;
        }

        if (orgPkList.containsKey(elem.hashCode())) {

            List<String> loopElems = new ArrayList<>();
            boolean found = false;
            for (Integer id : orgPkList.keySet()) {
                if (found) {
                    loopElems.add(orgPkList.get(id));
                } else if (id == elem.hashCode()) {
                    found = true;
                    loopElems.add(elem.getIdentifiableName());
                }
            }

            throw new EDCIConflictException("exception.client.error.msg.builder.conflict.full.loop", "'" + elem.getIdentifiableName() + "'",
                    loopElems.stream().collect(Collectors.joining(" -> ", "'", "'")) + " -> '" + elem.getIdentifiableName() + "')");

        } else {
            orgPkList.put(elem.hashCode(), elem.getIdentifiableName());
        }

        List<Identifiable> relElems = findAllObjectsIn(elem, Identifiable.class);

        if (relElems != null && relElems.size() > 0) {
    ...
    public <T> List<T> getCollectionFromMethod(Object root, Class<T> type, Method m) {
        ...
        try {
            ...
            if (col != null && !col.isEmpty()) {
                ...
                if (type.isInstance(col.iterator().next())) {
                    elems.addAll(col);
                }
            }
    ...
    public <T> List<T> findAllObjectsInRecursivily(Object root, Class<T> searchElems, Class... pathElems) {
        ...
        findAllObjectsInRecursivily(root, identifiebleObj, searchElems, pathElems);
        return identifiebleObj;
    }

    /* Warning, the result list is the auxList, not the return*/
    protected <T, R> List<R> findAllObjectsInRecursivily(Object root, List<T> auxField, Class<T> searchElems, Class<R>... pathElems) {
        ...
        for (R aux : identifiebleObj) {
            identifiebleObj = findAllObjectsInRecursivily(aux, auxField, searchElems, pathElems);
        }
