...
public class ImageUtil {
    ...
    protected static final Logger logger = LogManager.getLogger(ImageUtil.class);

    //Height and width in PX
    public static final int LOGO_HEIGHT = 200;
    public static final int LOGO_WIDTH = 0; //Proporcional to height

    public static final int BACKGROUND_HEIGHT = 1754; //For 150 ppi
    public static final int BACKGROUND_WIDTH = 0; //Proporcional to height
...
//    }
     ...
     * @throws IOException
     */
    public byte[] htmlToImageOld(String html) throws IOException {

        String header = "<head>\n" +
                " <style type=\"text/css\"> \n" +
                "     @page { size: " + EDCIConfig.Defaults.DIPLOMA_PAGE_SIZE + "; margin: " + EDCIConfig.Defaults.DIPLOMA_PAGE_MARGINS + "; } \n" +
                "</style>\n" +
                "</head>\n";
        ...
        if (imageDownloadURL != null && !imageDownloadURL.isEmpty()) {
            try {
                byte[] bytes = new EDCIRestRequestBuilder(HttpMethod.POST, imageDownloadURL)
                ...
                logger.error("Error generating the image when calling to " + imageDownloadURL, e);
                jpegImage = get503ImgError();
            }
        } else {
            jpegImage = get503ImgError();
        }
     ...
     *
     * @param html       html used for the image generation
     * @param background Size of the Image generated. If the size is smaller than the resulting image, this one will be split into different pages
     * @return
     * @throws IOException
     */
    public byte[] htmlToImage(String html, String background) throws IOException {
        String header = "<head>\n" +
                " <style type=\"text/css\"> \n" +
                "     @page { " +
                "        size: " + EDCIConfig.Defaults.DIPLOMA_PAGE_SIZE + "; " +
                "        margin: " + EDCIConfig.Defaults.DIPLOMA_PAGE_MARGINS + "; " +
                "        background: url(" + background + ") no-repeat center; background-size: contain; } \n" +
                "</style>\n" +
                "</head>\n";
            ...
            try {
                ...
                try (ByteArrayInputStream in = new ByteArrayInputStream(bytes); ByteArrayOutputStream buffer = new ByteArrayOutputStream()) {
                    ...
                    BufferedImage result = new BufferedImage(
                            ...
                            BufferedImage.TYPE_INT_RGB);
                    result.createGraphics().drawImage(image, 0, 0, Color.WHITE, null);
                    ImageIO.write(result, ControlledListsUtil.MimeType.JPG.getExtension(), buffer);
                    jpegImage = buffer.toByteArray();
                }
    ...
    public byte[] get503ImgError() {
        try {
            byte[] jpegImage = null;
            byte[] bytes = IOUtils.toByteArray(new ClassPathResource("img/503_service.jpg").getInputStream());
    ...
    public byte[] resizeImage(byte[] imageData, String type, int targetHeightpx, int targetWidthpx) throws IOException {
        ...
        try (ByteArrayInputStream in = new ByteArrayInputStream(imageData); ByteArrayOutputStream buffer = new ByteArrayOutputStream()) {
            ...
            if (img == null) {
                throw new EDCIException().addDescription("Invalid image");
            }
...
//        }
    ...
    public ConceptDTO getBase64Encoding() {
