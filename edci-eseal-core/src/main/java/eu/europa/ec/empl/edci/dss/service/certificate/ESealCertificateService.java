package eu.europa.ec.empl.edci.dss.service.certificate;
...
@Service
public class ESealCertificateService {

    private static final Logger logger = LogManager.getLogger(ESealCertificateService.class);
    ...
    public Map<String, String> getCertificateInfo(String certPath, String password) {
        ...
        try {
            ...
            certInfo.put(ESealConstants.Certificate.CERTIFICATE_ATTRIBUTE_COUNTRY_NAME, country);
            certInfo.put(ESealConstants.Certificate.CERTIFICATE_ATTRIBUTE_COMMON_NAME, commonName);
            certInfo.put(ESealConstants.Certificate.CERTIFICATE_ATTRIBUTE_ORGANIZATION, organization);
            certInfo.put(ESealConstants.Certificate.CERTIFICATE_ATTRIBUTE_ORGANIZATION_IDENTIFIER, organizationID);
        } catch (CertificateEncodingException e) {
            throw new ESealException(HttpStatus.INTERNAL_SERVER_ERROR, ErrorCode.CANNOT_READ_LOCAL_CERTIFICATE, ESealMessageKeys.Exception.ESeal.CANNOT_READ_LOCAL_CERTIFICATE).setCause(e);
        }
        return certInfo;
    }
    ...
    public Map<String, String> getCertificateInfo(String certificate) {
        ...
        try {
            ...
            oidMap.put("2.5.4.97", ESealConstants.Certificate.CERTIFICATE_ATTRIBUTE_ORGANIZATION_IDENTIFIER);
            String issuerName = subject.getName(X500Principal.RFC2253, oidMap);

            List<String> values = Arrays.asList(issuerName.split(ESealConstants.StringPool.STRING_COMMA));
            for (String value : values) {
                String[] attributeNameAndValue = value.split(ESealConstants.StringPool.STRING_EQUALS);
    ...
    public X509Certificate getCertificate(String certPath, String password) {
        try {
            ...
            if (keystore.size() > 1) {
                throw new ESealException(HttpStatus.INTERNAL_SERVER_ERROR, ErrorCode.CANNOT_READ_LOCAL_CERTIFICATE, ESealMessageKeys.Exception.ESeal.CANNOT_READ_LOCAL_CERTIFICATE);
            }
            ...
            throw new ESealException(HttpStatus.INTERNAL_SERVER_ERROR, ErrorCode.LOCAL_CERTIFICATE_NOT_FOUND, ESealMessageKeys.Exception.ESeal.LOCAL_CERTIFICATE_NOT_FOUND, certPath).setCause(e);
        } catch (IOException e) {
            throw new ESealException(HttpStatus.BAD_REQUEST, ErrorCode.LOCAL_CERTIFICATE_BAD_PASSWORD, ESealMessageKeys.Exception.ESeal.LOCAL_CERTIFICATE_BAD_PASSWORD).setCause(e);
        }
    }
