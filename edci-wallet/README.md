...
#### Tech stack
##### Front-end
Angular 13.2 (eUI v13.1.2 - EUI Framework) available, but not currently used.
##### Back-end
Java 11, 
Spring,
Titanium,
...
```
## Core functions
In this section, the more important functionalities of the Wallet are described. 
...
### Add Credential
The EDCI wallet project provides a way to add a credential to an  based on a user ID and an JSON-LD File. Before storing the JSON-LD file, the wallet will first check that it is signed and the signature is a qualified qseal, and depending no configuration it will be allowed or not.
It is required the the uploadad credential follows the [european learning model](https://github.com/european-commission-europass/Europass-Learning-Model) framework.
...
### Get Credential
The basic function of the wallet is to serve stored JSON-LD credentials that reside in the wallets to different applications that make use of those. 
...
By default, this mock user will be enabled with the property "oidc.mock.user.active" set to true. This property can be found into the "security.properties" file and should be set to false before enabling the desired authentication.
...
* European Learning Model can be found [here](https://github.com/european-commission-europass/Europass-Learning-Model).