...
public class CredentialResource implements CrudResource {
    ...
    private static final Logger logger = LogManager.getLogger(CredentialResource.class);
...
//                    , hateoasPresentation
    ...
    public ResponseEntity<ByteArrayResource> downloadVerifiablePresentationPDFFromFile(@RequestPart(value = EDCIParameter.WALLET_CREDENTIAL_FILE) MultipartFile file,
        ...
        return credentialService.downloadVerifiablePresentationPDF(europassPresentationDTO, null, pdfType, null);
    ...
    public ResponseEntity<byte[]> downloadDiploma(
        ...
        walletService.validateWalletExists(userId);
        byte[] diplomaJPG = imageUtil.resizeImage(credentialService.getDiplomaImage(userId, uuid).get(0), ControlledListsUtil.MimeType.JPG.getExtension(), 750, 0);
    ...
    public ResponseEntity<Resource<EuropassDiplomaDTO>> downloadDiplomaMultipage(
        ...
        CredentialHolderDTO cred = edciCredentialModelUtil.fromByteArray(credentialDTO.getCredentialXML());

        return generateOkResponse(diplomaUtils.generateEuropassDiplomaDTO(cred, credentialDTO.getDiplomaImage(), locale));
    }
