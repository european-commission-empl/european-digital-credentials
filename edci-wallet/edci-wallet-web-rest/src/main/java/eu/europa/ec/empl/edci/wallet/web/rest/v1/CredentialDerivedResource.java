...
public class CredentialDerivedResource implements CrudResource {
    ...
    public ResponseEntity<Resource<ShareLinkInfoView>> createShareLink(
        ...
        walletService.validateWalletUserIdExists(userId);

        WalletDTO walletDTO = walletService.fetchWalletByUserId(userId);

        ShareLinkInfoView shareResponse = shareLinkRestMapper.toResponseVO(shareLinkService.createShareLinkWalletAddress(walletDTO.getWalletAddress(),
...
//                                                       @ApiParam(required = false, value = "The information that we want into the PDF: full/diploma. By default Diploma", defaultValue = "diploma") @RequestParam(value = Parameter.PDF_TYPE, required = false) String pdfType,
        ...
        WalletDTO walletDTO = walletService.fetchWalletByUserId(userId);
        return credentialService.downloadVerifiablePresentationPDF(credentialService.fetchCredentialByUUID(walletDTO.getWalletAddress(), credential.getUuid()), expirationDate, pdfType);
...
//    }
    ...
    public ResponseEntity<byte[]> getThumbnail(
        ...
        WalletDTO walletDTO = walletService.fetchWalletByUserId(userId);
        byte[] diplomaImageFull = DatatypeConverter.parseBase64Binary(new String(credentialService.getDiplomaImage(walletDTO.getWalletAddress(), uuid, locale).get(0)).split("base64,")[1]);
    ...
    public ResponseEntity<Resource<EuropassDiplomaDTO>> downloadDiplomaMultipage(
        ...
        CredentialDTO credentialDTO = credentialService.fetchCredentialByUUID(walletDTO.getWalletAddress(), uuid);
        ...
        diploma.setBase64DiplomaImages(credentialService.getDiplomaImage(credentialDTO.getWallet().getWalletAddress(), uuid, locale).stream().map(String::new).collect(Collectors.toList()));
