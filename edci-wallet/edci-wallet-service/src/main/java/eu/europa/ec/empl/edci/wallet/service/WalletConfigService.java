    ...
    public String getViewerURL(CredentialDTO credentialDTO) {
        ...
        try {
            viewerURL = getString(EDCIWalletConstants.CONFIG_PROPERTY_VIEWER_VIEW_ADDRESSS)
                    .concat(credentialDTO.getWallet().getUserId()).concat(EDCIWalletConstants.STRING_SLASH).concat(String.valueOf(URLEncoder.encode(String.valueOf(credentialDTO.getUuid()), StandardCharsets.UTF_8.name())));
