...
the EDC to support a customised look and feel. The EDCI Viewer displays an HTML representation of the credential, so that is why this 
custom credential will be built in HTML + CSS (no JS) inside the EDC's XML structure, and will use wildcards to choose EDC content 
...
Full example is showcased in [Annex I](#Annex I).
...
Important: The provided HTML will be sanitised (automatic removal) to prevent Cross site scripting when uploaded. 
See the tags and attributes allowed in the [Annex II](#Annex II). For more information: https://owasp.org/www-project-java-html-sanitizer/.

## Wildcards use

There are two different kinds of wildcards: the ones extracting data from the Europass Learning Model, and the ones to display custom labels.
