...
import { CredentialView, CustomizableSpecView, SignatureNexuView, UserDetailsView, V1Service } from '@shared/swagger';
...
export class IssuerService {
    ...
    public downloadCustomizableTemplate(oid: number, locale: string, customizableSpecView: CustomizableSpecView) {
        this.api.getCredentialSpecXLSTemplate(oid, locale, customizableSpecView, 'response').subscribe({
            next : (response) => {
        ...
        }).add(() => this.isExcelDownloadedSubject.next(false));
    }
