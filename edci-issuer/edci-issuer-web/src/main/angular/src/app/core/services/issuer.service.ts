...
export class IssuerService {
    ...
    public breadcrumbSubject: Subject<string> = new Subject<string>();
    public isExcelDownloadedSubject: Subject<boolean> = new Subject<boolean>();
    ...
    public getFile(): DocumentUpload {
        return this.file;
    }

    public setFile(file: DocumentUpload) {
        this.file = file;
    }
    ...
    public downloadRecipientTemplate(oid: number, locale: string) {
        this.api
            .getRecipientTemplate(oid, locale, 'response')
            .subscribe((x) => {
    ...
    downloadCSV(arg: { fileName: string, text: string }) {
        ...
        element.setAttribute('href', `data:${fileType};charset=UTF-8,%EF%BB%BF${encodeURIComponent(arg.text)}`);
    ...
    generateUTCDate(date: Date, time: boolean): string {
        ...
        const dateToReturn = ('0' + date.getDate()).slice(-2) + '-'
                    + ('0' + (date.getMonth() + 1)).slice(-2) + '-'
                    + date.getFullYear();
        if (!time) {
            return dateToReturn;
        }
        return dateToReturn + `T${date.getUTCHours()}:${date.getUTCMinutes()}:${date.getUTCSeconds()}`;
    }
