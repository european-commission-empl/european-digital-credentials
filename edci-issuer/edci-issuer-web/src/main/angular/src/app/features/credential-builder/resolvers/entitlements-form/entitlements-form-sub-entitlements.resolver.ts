...
export class EntitlementsFormSubEntitlementsResolver implements Resolve<PagedResourcesEntitlementSpecLiteView> {
    constructor(
    ...
    resolve(route: ActivatedRouteSnapshot): Observable<PagedResourcesEntitlementSpecLiteView> {
        ...
        if (routeID && currentLanguage) {
            return this.api.listHasEntPart(routeID, currentLanguage).pipe(
                take(1),
                catchError(() => {
