...
export class EntitlementsFormValidWithResolver implements Resolve<PagedResourcesOrganizationSpecLiteView> {
    constructor(
    ...
    resolve(route: ActivatedRouteSnapshot): Observable<PagedResourcesOrganizationSpecLiteView> {
        ...
        if (routeID && currentLanguage) {
            return this.api.getValidWith(routeID, currentLanguage).pipe(
                take(1),
                catchError(() => {
