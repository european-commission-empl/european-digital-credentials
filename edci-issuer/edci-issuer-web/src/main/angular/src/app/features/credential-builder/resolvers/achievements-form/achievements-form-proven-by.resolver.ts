...
export class AchievementsFormProvenByResolver implements Resolve<PagedResourcesAssessmentSpecLiteView> {
    constructor(
    ...
    resolve(route: ActivatedRouteSnapshot): Observable<PagedResourcesAssessmentSpecLiteView> {
        ...
        if (routeID && currentLanguage) {
            return this.api.getProvenBy(routeID, currentLanguage).pipe(
                take(1),
                catchError(() => {
