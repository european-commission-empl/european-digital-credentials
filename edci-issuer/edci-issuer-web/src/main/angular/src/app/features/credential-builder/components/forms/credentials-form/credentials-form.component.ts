...
import { Observable, Subject, debounceTime, distinctUntilChanged, switchMap, of, catchError, finalize } from 'rxjs';
...
export class CredentialsFormComponent implements OnInit, OnDestroy {
    ...
    isLoading = true;
    learnLink = environment.accreditationLearnLink;
    language: string;
    maxDate: Date;
    minDate: Date;
    ...
    ngOnInit() {
        ...
        this.formGroup.get('accreditation').valueChanges.pipe(
            ...
            switchMap(val => {
                ...
                this.formGroup.get('accreditation')?.setErrors(null);
                if (val) {
                    return this.api.doesAccreditationExist(val, this.language, 'response').pipe(catchError((err) => {
                        return of(err);
                    }));
                } else {
                    return of(null);
            ...
            next: (res) => {
                ...
                if (res !== null && res?.status !== 204) {
                    this.formGroup.get('accreditation')?.setErrors({ invalid: true });
                }
            }
        });
    ...
    private setForm(): void {
        ...
        this.formGroup.patchValue({
            ...
            accreditation : this.editCredential.hasAccreditation
        });
    }
