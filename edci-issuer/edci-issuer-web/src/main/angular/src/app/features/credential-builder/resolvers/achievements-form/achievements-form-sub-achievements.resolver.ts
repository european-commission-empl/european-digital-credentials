...
export class AchievementsFormSubAchievementsResolver implements Resolve<PagedResourcesLearningAchievementSpecLiteView> {
    constructor(
    ...
    resolve(route: ActivatedRouteSnapshot): Observable<PagedResourcesLearningAchievementSpecLiteView> {
        ...
        if (routeID && currentLanguage) {
            return this.api.listSubAchievements(routeID, currentLanguage).pipe(
                take(1),
                catchError(() => {
