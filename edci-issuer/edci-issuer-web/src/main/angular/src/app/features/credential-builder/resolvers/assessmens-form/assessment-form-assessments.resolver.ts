...
export class AssessmentFormAssessmentsResolver implements Resolve<PagedResourcesOrganizationSpecLiteView> {
    constructor(
    ...
    resolve(route: ActivatedRouteSnapshot): Observable<PagedResourcesOrganizationSpecLiteView> {
        ...
        if (routeID && currentLanguage) {
            return this.api.listAssessedBy(routeID, currentLanguage).pipe(
                take(1),
                catchError(() => {
