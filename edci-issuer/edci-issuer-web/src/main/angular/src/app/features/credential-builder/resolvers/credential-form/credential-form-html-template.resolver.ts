...
import { DiplomaSpecLiteView, V1Service } from '@shared/swagger';
...
export class CredentialFormHtmlTemplateResolver implements Resolve<DiplomaSpecLiteView> {
    constructor(
    ...
    resolve(route: ActivatedRouteSnapshot): Observable<DiplomaSpecLiteView> {
        ...
        if (routeID && currentLanguage) {
            return this.api.getDiploma(routeID, currentLanguage).pipe(
                take(1),
                catchError(() => {
