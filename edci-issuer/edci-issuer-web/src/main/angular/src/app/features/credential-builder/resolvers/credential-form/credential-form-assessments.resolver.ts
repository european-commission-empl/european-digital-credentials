...
export class CredentialFormAssessmentsResolver implements Resolve<PagedResourcesEntitlementSpecLiteView> {
    constructor(
    ...
    resolve(route: ActivatedRouteSnapshot): Observable<PagedResourcesEntitlementSpecLiteView> {
        ...
        if (routeID && currentLanguage) {
            return this.api.listAssessed(routeID, currentLanguage).pipe(
                take(1),
                catchError(() => {
