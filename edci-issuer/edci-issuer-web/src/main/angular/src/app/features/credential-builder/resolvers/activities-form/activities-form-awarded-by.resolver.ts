...
export class ActivitiesFormAwardedByResolver implements Resolve<PagedResourcesOrganizationSpecLiteView> {
    constructor(
    ...
    resolve(route: ActivatedRouteSnapshot): Observable<PagedResourcesOrganizationSpecLiteView> {
        ...
        if (routeID && currentLanguage) {
            return this.api.listAwardingBodiesAct(routeID, currentLanguage).pipe(
                take(1),
                catchError(() => {
