...
export class CredentialFormEntitledToResolver implements Resolve<PagedResourcesEntitlementSpecLiteView> {
    constructor(
    ...
    resolve(route: ActivatedRouteSnapshot): Observable<PagedResourcesEntitlementSpecLiteView> {
        ...
        if (routeID && currentLanguage) {
            return this.api.listEntitledTo(routeID, currentLanguage).pipe(
                take(1),
                catchError(() => {
