...
export class EntitlementsFormAwardedByResolver implements Resolve<PagedResourcesOrganizationSpecLiteView> {
    constructor(
    ...
    resolve(route: ActivatedRouteSnapshot): Observable<PagedResourcesOrganizationSpecLiteView> {
        ...
        if (routeID && currentLanguage) {
            return this.api.listAwardingBodiesEnt(routeID, currentLanguage).pipe(
                take(1),
                catchError(() => {
