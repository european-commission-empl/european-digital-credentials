...
export class AccreditationFormAccreditingAgentResolver implements Resolve<ResourceOrganizationSpecView> {
    constructor(
    ...
    resolve(route: ActivatedRouteSnapshot): Observable<ResourceOrganizationSpecView> {
        ...
        if (routeID && currentLanguage) {
            return this.api.getAccreditingAgent(routeID, currentLanguage).pipe(
                take(1),
                catchError(() => {
