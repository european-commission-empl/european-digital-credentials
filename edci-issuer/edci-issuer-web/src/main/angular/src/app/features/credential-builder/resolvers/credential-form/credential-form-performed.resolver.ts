...
export class CredentialFormPerformedResolver implements Resolve<PagedResourcesLearningActivitySpecLiteView> {
    constructor(
    ...
    resolve(route: ActivatedRouteSnapshot): Observable<PagedResourcesLearningActivitySpecLiteView> {
        ...
        if (routeID && currentLanguage) {
            return this.api.listPerformed(routeID, currentLanguage).pipe(
                take(1),
                catchError(() => {
