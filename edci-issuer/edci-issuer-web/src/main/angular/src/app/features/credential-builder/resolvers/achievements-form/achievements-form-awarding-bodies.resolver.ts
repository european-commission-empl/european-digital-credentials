...
export class AchievementsFormAwardingBodiesResolver implements Resolve<PagedResourcesOrganizationSpecLiteView> {
    constructor(
    ...
    resolve(route: ActivatedRouteSnapshot): Observable<PagedResourcesOrganizationSpecLiteView> {
        ...
        if (routeID && currentLanguage) {
            return this.api.listAwardingBodiesAch(routeID, currentLanguage).pipe(
                take(1),
                catchError(() => {
