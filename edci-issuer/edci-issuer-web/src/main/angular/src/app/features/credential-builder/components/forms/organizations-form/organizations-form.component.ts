...
export class OrganizationsFormComponent implements OnInit, OnDestroy {
    ...
    selectedAccreditations: PagedResourcesAccreditationSpecLiteView = {
        content: [],
        links: [],
        page: null,
    };
    ...
    formGroup = new FormGroup({
        ...
        contactPoints: this.fb.array([
            this.fb.group(
                ...
                {
                    validators: [
                        this.contactPointValidator.emailValidator(),
                        this.contactPointValidator.countryValidator(),
                    ],
                    updateOn: 'blur',
                }
            ),
    ...
    ngOnInit() {
        this.route.data.pipe(takeUntil(this.destroy$)).subscribe((data) => {
            ...
            if (this.isModal) {
                ...
                if (this.modalData?.organizationDetails) {
                    this.setEditOrganizationFormData(
                        this.modalData.organizationDetails
                    );

                    if (this.modalData?.accreditation) {
                        this.selectedAccreditations =
                            this.modalData.accreditation;
                        const oids = this.modalData.accreditation?.content?.map(
                            (item) => item.oid
                        );
                        this.accreditation.patchValue(oids);
                    }
                } else {
                ...
                if (data?.accreditation) {
                    this.selectedAccreditations = data?.accreditation;
                    const oids = data?.accreditation?.content?.map(
                        (item) => item.oid
                    );
                    this.accreditation.patchValue(oids);
                }
    ...
    onAccreditationSelectionChange(oids): void {
        this.accreditation.markAsTouched();
        this.accreditation.patchValue(oids);
    }
    ...
    addLocationsLanguageControls(language: string) {
        this.location.controls.forEach((location) => {
            this.addLocationGroupLanguageControls(
                language,
                location as FormGroup
            );
        });
    }

    addLocationGroupLanguageControls(
        language: string,
        ...
        contactPoint.forEach((element, index) => {
            ...
            if (element?.description?.contents[0]?.content) {
                this.contactPoints
                    .at(index)
                    .get('description')
                    .patchValue(element?.description?.contents[0]?.content);
            }
            /* multilang */
            if (element?.address[0]?.fullAddress?.contents[0]?.content) {
                this.contactPoints
                    ...
                    .get('address')
                    .patchValue(
                        element?.address[0].fullAddress.contents[0].content
                    );
            }
            ...
            if (element?.address[0]?.countryCode) {
                this.contactPoints
                    .at(index)
                    .get('country')
                    .patchValue(element?.address[0].countryCode);
            }
            ...
            if (element?.emailAddress[0]?.id) {
                this.contactPoints
                    .at(index)
                    .get('email')
                    .patchValue(element?.emailAddress[0]?.id);
            }
            if (element?.contactForm[0]?.contentUrl) {
                this.contactPoints
                    .at(index)
                    .get('contactForm')
                    .patchValue(element?.contactForm[0]?.contentUrl);
            }
            if (element?.phone[0]?.phoneNumber) {
                this.contactPoints
                    .at(index)
                    .get('phone')
                    .patchValue(element?.phone[0]?.phoneNumber);
            }
        });
    }
    ...
    updateOrganizationAndLogo() {
        ...
        const background$ = template$.pipe(
            switchMap((templateDetails) =>
                this.api.addLogo(templateDetails.oid, this.logo).pipe(take(1))
            )
        );
    ...
    private setIdentifierErrorsObservable() {
        this.vatIdentifier.valueChanges
            .pipe(takeUntil(this.destroy$))
            .subscribe((res) => {
        ...
        this.taxIdentifier.valueChanges
            .pipe(takeUntil(this.destroy$))
            .subscribe((res) => {
                if (res.content !== null && res.spatialId === null) {
    ...
    private saveForm(): void {
        if (this.formGroup.invalid) {
            ...
            if (
                ...
                (this.logo || this.base64FromRequest)
            ) {
    ...
    private addAdditionalInformationControl(
        language: string,
        value = ''
    ): void {
        this.additionalInformation.addControl(
            language,
            new FormControl(
                value,
                Validators.maxLength(Constants.MAX_LENGTH_LONG)
                ...
                (identifier: IdentifierDTView) => {
                    this.identifiers.push(
                        new FormGroup(
                            {
                                id: new FormControl(
                                    _get(identifier, 'notation', null),
                                    Validators.maxLength(
                                        Constants.MAX_LENGTH_DEFAULT
                                    )
                                ),
                                identifierSchemeAgencyName: new FormControl(
                                    _get(identifier, 'schemeName', null),
                                    Validators.maxLength(
                                        Constants.MAX_LENGTH_DEFAULT
                                    )
                                ),
        ...
        if (groups && groups.length > 0) {
            ...
            groups.forEach((identifier: GroupDCView) => {
                ...
                const nameContents: ContentDTView[] =
                    identifier.prefLabel.contents;

                const nameGroup = new FormGroup({});
                nameContents.forEach((content) =>
                    nameGroup.addControl(
                        content.language,
                        new FormControl(content.content, [
                            Validators.maxLength(Constants.MAX_LENGTH_DEFAULT),
                            Validators.required,
                            noSpaceValidator,
                        ])
    ...
    private setOrganizationBody(): void {
        this.organizationBody = {
            ...
            relAccreditation:
                this.accreditation.value && this.accreditation.value
                    ? {
                          oid: this.accreditation.value,
                      }
                    : null,
            defaultLanguage: this.defaultLanguage,
            ...
            location: this.formsService.getLocationDCView(this.location)
                ? [this.formsService.getLocationDCView(this.location)]
                : [],
            additionalInfo: {
            ...
            additionalNote: this.setPayloadLabel(
    ...
    private setModifiedDatePayload(): string {
        const date = this.dateFormatService.dateToStringDateTime(
    ...
    private setPayloadLabel(value): TextDTView[] {
        const valueContent: ContentDTView[] =
            this.multilingualService.formToView(value);
    ...
    private setForm(): void {
        ...
        this.formGroup.patchValue({
            ...
            eIDAS: {
                content: _get(
                    this.editOrganization,
                    'eidasIdentifier.notation',
                    null
                ),
                spatialId: _get(
                    this.editOrganization,
                    'eidasIdentifier.spatialId',
                    null
                ),
            },
            vatIdentifier: {
            ...
            email: _get(
                this.editOrganization,
                'contactPoint[0].emailAddress[0].id',
                null
            ),
            phone: _get(
                this.editOrganization,
                'contactPoint[0].phone[0].phoneNumber',
                null
            ),
            contactForm: _get(
                this.editOrganization,
                'contactPoint[0].contactForm[0].contentUrl',
                null
            ),
            homePage: _get(
                this.editOrganization,
        ...
        this.addLocationControls(
            _get(this.editOrganization, 'location[0]', null)
        );
        this.isLoading = false;
    }
    ...
    private addLocationControls(location: LocationDCView): void {
        if (location) {
            ...
            const fullAddressContents =
                location.address[0]?.fullAddress?.contents;
            ...
            this.selectedLanguages.forEach((lang) => {
                ...
                addrCtrl.addControl(
                    langCode,
                    new FormControl(fullAddressLangContent, [
                        Validators.maxLength(Constants.MAX_LENGTH_LONG),
                    ])
                );
                descCtrl.addControl(
                    langCode,
                    new FormControl(descriptionLangContent, [
                        Validators.maxLength(Constants.MAX_LENGTH_LONG),
                    ])
                );
            });
            ...
            locationGroup
                .get('country')
                .patchValue(location.address[0]?.countryCode);
    ...
    private addControlsFromView(): void {
        this.languages.forEach((language: string) => {
            this.addPreferredNameControl(
                ...
                this.multilingualService.getContentFromLanguage(
                    language,
                    _get(this.editOrganization, 'legalName.contents', [])
                )
            );
            ...
            this.addAlternativeNameControl(
                ...
                this.multilingualService.getContentFromLanguage(
                    language,
                    _get(this.editOrganization, 'altLabel.contents', [])
                )
            );
