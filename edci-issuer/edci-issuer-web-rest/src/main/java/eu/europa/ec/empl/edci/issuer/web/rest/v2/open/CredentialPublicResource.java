package eu.europa.ec.empl.edci.issuer.web.rest.v2.open;
...
@RequestMapping(EDCIConstants.Version.V2 + IssuerEndpoint.V1.PUBLIC + IssuerEndpoint.V1.CREDENTIALS)
@Controller
public class CredentialPublicResource {
    ...
    public ResponseEntity<?> batchSealAndSendCredentials(@ApiParam(required = true, value = "the JSON files containing the credential") @RequestPart(Parameter.FILES) MultipartFile[] files,
        ...
        ApiErrorMessage apiErrorMessage = this.getCredentialPublicService().doBatchSealAndSendCredentials(batchId, new PublicBatchSealingDTO(files, password, signOnBehalf));
    ...
    @ApiOperation(value = "Seal a credential in JSON format and download resulting file. Requires a configured local certificate.", response = File.class)
    @PostMapping(value = IssuerEndpoint.V1.CREDENTIALS_SEAL,
            consumes = MediaType.MULTIPART_FORM_DATA_VALUE,
            produces = MediaType.APPLICATION_XML_VALUE)
    public ResponseEntity<byte[]> sealAndDownloadCredential(@ApiParam(required = true, value = "The JSON file containig the credential") @RequestPart(Parameter.FILE) MultipartFile file,
                                                            @ApiParam(required = true, value = "The password for the local certificate ") @RequestParam(value = Parameter.PASSWORD) String password,
                                                            @ApiParam(required = false, value = "indicates that the credential should be signed on behalf of another organization") @RequestParam(value = Parameter.SIGN_ON_BEHALF, required = false, defaultValue = "false") boolean signOnBehalf) {
        PublicSealingDTO publicSealingDTO = new PublicSealingDTO(file, password, signOnBehalf);
        return new ResponseEntity<>(this.getCredentialPublicService().doLocalSignOrCreateAndDownloadCredential(publicSealingDTO, true),
                prepareHttpHeadersForCredentialDownload(file.getName(), MediaType.APPLICATION_XML_VALUE),
                HttpStatus.OK);

    }

    @ApiOperation(value = "Create a credential in JSON format and download resulting file.", response = File.class)
    @PostMapping(value = IssuerEndpoint.V1.CREDENTIALS_CREATE,
            consumes = MediaType.MULTIPART_FORM_DATA_VALUE,
            produces = MediaType.APPLICATION_XML_VALUE)
    public ResponseEntity<byte[]> createAndDownloadCredential(@ApiParam(required = true, value = "The JSON file containig the credential") @RequestPart(Parameter.FILE) MultipartFile file) {
        PublicSealingDTO publicSealingDTO = new PublicSealingDTO(file, "", false);
        return new ResponseEntity<>(this.getCredentialPublicService().doLocalSignOrCreateAndDownloadCredential(publicSealingDTO, false),
                prepareHttpHeadersForCredentialDownload(file.getName(), MediaType.APPLICATION_XML_VALUE),
                HttpStatus.OK);

    }

    @ApiOperation(value = "Seal a credential in JSON format, using the locally stored cert, and sent it to the wallet. Requires a configured local certificate.")
    @PostMapping(value = IssuerEndpoint.V1.CREDENTIALS_SEAL_SEND,
            consumes = MediaType.MULTIPART_FORM_DATA_VALUE,
            produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public PublicSealAndSendView sealAndSendCredential(@ApiParam(required = true, value = "The JSON file containing the credential") @RequestPart(Parameter.FILE) MultipartFile file,
                                                       ...
                                                       @ApiParam(required = false, value = "indicates that the credential should be signed on behalf of another organization") @RequestParam(value = Parameter.SIGN_ON_BEHALF, required = false, defaultValue = "false") boolean signOnBehalf) {
        PublicSealingDTO publicSealingDTO = new PublicSealingDTO(file, password, signOnBehalf);
        return this.getCredentialRestMapper().toVO(this.getCredentialPublicService().doSealAndSendCredentials(publicSealingDTO));
