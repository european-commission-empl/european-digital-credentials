    ...
    public ResponseEntity<Page> listEntities(
        ...
        if ("skill".equalsIgnoreCase(type) || "occupation".equalsIgnoreCase(type)) {
            ...
            if (ControlledList.ISCED_F.getName().equalsIgnoreCase(type)) {
                List<CodeDTView> elems = rdfSparqlBridgeService.searchISCEDFTreeConcepts(clSel.getUrl(), search, LocaleContextHolder.getLocale().getLanguage(), reqLangs)
                        .stream()
                        .map(rdfConcept -> genericEntitiyRestMapper.toCodeView(rdfSparqlBridgeService.toConceptDTO(rdfConcept)))
                        .collect(Collectors.toList());
                returnValue = new PageImpl<CodeDTView>(elems, pageParam.toPageRequest(), elems.size());
            } else if (ControlledList.NQF.getName().equalsIgnoreCase(type) || !StringUtils.isEmpty(parent)) {
                List elems = rdfSparqlBridgeService.searchBroaderConcepts(clSel.getUrl(), parent, search, LocaleContextHolder.getLocale().getLanguage(), page, size, reqLangs)
                        .stream()
                        .map(rdfConcept -> genericEntitiyRestMapper.toCodeView(rdfSparqlBridgeService.toConceptDTO(rdfConcept)))
                        .collect(Collectors.toList());
                returnValue = new PageImpl<CodeDTView>(elems, pageParam.toPageRequest(), elems.size());
