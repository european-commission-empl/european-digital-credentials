...
//        public ResponseEntity<Page> listCredential (@ModelAttribute PageParam pageParam) throws Exception { //Swagger expects pageParam to be in body. don't be like Swagger
    ...
    public ResponseEntity<byte[]> getRecipientTemplate(
        ...
        return credentialService.generateRecipientXLS(oid);
    }
    ...
    @ApiOperation(value = "Generates and returns the recipient's customizable fields")
    @GetMapping(value = IssuerEndpoint.V1.SPECS + IssuerEndpoint.V1.RECIPIENTS + IssuerEndpoint.V1.FIELDS,
            produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public ResponseEntity<Resource<String>> getRecipientCustomizableFields(
            @ApiParam(value = "locale") @RequestParam(value = Parameter.LOCALE, required = false) String locale
    ) throws Exception {

        String recipientView = null; //CustomizableRecipientView

        //Call EPASS-435 service here

        return generateOkResponse(recipientView);
    }
