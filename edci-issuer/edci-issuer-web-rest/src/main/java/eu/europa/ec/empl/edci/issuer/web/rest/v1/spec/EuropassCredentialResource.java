    ...
    public ResponseEntity<Resource<EuropassCredentialSpecView>> createCredential(@RequestBody @Valid EuropassCredentialSpecView credentialView,
        ...
        EuropassCredentialSpecDAO credentialCreatedDAO = credentialService.save(credentialDAO,
                () -> {
                    ...
                    credentialDAO.setAssessedBy(assessmentSpecService.retrieveEntities( false, credentialView.getRelAssessed().getOid()));
...
//        public ResponseEntity<Page> listCredential (@ModelAttribute PageParam pageParam) throws Exception { //Swagger expects pageParam to be in body. don't be like Swagger
    ...
    public ResponseEntity<PagedResources<LearningAchievementSpecLiteView>> listAchieved(
        ...
        return generateListResponse(credentialDAO.getAchieved(), "/specs", learningAchievementSpecRestMapper, LocaleContextHolder.getLocale().toString());
    ...
    public ResponseEntity<PagedResources<LearningActivitySpecLiteView>> listPerformed(
        ...
        return generateListResponse(credentialDAO.getPerformed(), "/specs", learningActivitySpecRestMapper, LocaleContextHolder.getLocale().toString());
    ...
    public ResponseEntity<PagedResources<EntitlementSpecLiteView>> listEntitledTo(
        ...
        return generateListResponse(credentialDAO.getEntitledTo(), "/specs", entitlementSpecRestMapper, LocaleContextHolder.getLocale().toString());
    ...
    @ApiOperation(value = "Gets a list of subresources")
    @GetMapping(value = IssuerEndpoint.V1.SPECS + Parameter.Path.OID + IssuerEndpoint.V1.CRED_ASSESSED_BY,
            produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    @PreAuthorize("isAuthenticated()")
    public ResponseEntity<PagedResources<AssessmentSpecLiteView>> listAssessed(
            @ApiParam(required = true, value = "The Credential oid") @PathVariable(Parameter.OID) Long oid,
            @ApiParam(value = "locale") @RequestParam(value = Parameter.LOCALE, required = false) String locale
    ) throws Exception {
        ...
        return generateListResponse(credentialDAO.getAssessedBy(), "/specs", assessmentSpecRestMapper, LocaleContextHolder.getLocale().toString());

    }

    @ApiOperation(value = "Link an existing subresource")
    @PostMapping(value = IssuerEndpoint.V1.SPECS + Parameter.Path.OID + IssuerEndpoint.V1.CRED_ASSESSED_BY,
            consumes = MediaType.APPLICATION_JSON_VALUE,
            produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    @PreAuthorize("isAuthenticated()")
    public ResponseEntity<PagedResources<AssessmentSpecLiteView>> setAssessedBy(
            @ApiParam(required = true, value = "The Credential oid") @PathVariable(Parameter.OID) Long oid,
            @RequestBody SubresourcesOids oids,
            @ApiParam(value = "locale") @RequestParam(value = Parameter.LOCALE, required = false) String locale) throws Exception {
        ...
        Set<LearningAssessmentSpecDAO> entities = retrieveEntities(assessmentSpecService, false, oids.getOid());

        credentialDAO.setAssessedBy(entities);
    ...
    public ResponseEntity<Resource<AssessmentsListIssueView>> getIssuerAssessmentGrades(
        ...
        Set<LearningAssessmentSpecDAO> assessments = credentialService.getCredentialAssessments(oid);
    ...
    public ResponseEntity<byte[]> getCredentialSpecXLSTemplate(@PathVariable(Parameter.OID) Long oid,
        ...
        String fileName = credentialDAO.getName().replaceAll("[^a-zA-Z0-9\\.\\-]", "_").concat(".xls");
    ...
    public Link[] generateCredentialHateoas(EuropassCredentialSpecDAO credentialDAO) {
        ...
        Link hateoasSelf = ControllerLinkBuilder.linkTo(EuropassCredentialResource.class).slash(IssuerEndpoint.V1.SPECS).slash(credentialDAO.getPk().hashCode()).withSelfRel();

        Link hateoasLearningAchievements = ControllerLinkBuilder.linkTo(EuropassCredentialResource.class)
                .slash(IssuerEndpoint.V1.SPECS).slash(credentialDAO.getPk().hashCode())
        ...
        Link hateoasLearningAssessments = ControllerLinkBuilder.linkTo(EuropassCredentialResource.class)
                .slash(IssuerEndpoint.V1.SPECS).slash(credentialDAO.getPk().hashCode())
                .slash(IssuerEndpoint.V1.CRED_ASSESSED_BY).withRel("assessedBy");
        ...
        return new Link[]{
                hateoasSelf, hateoasLearningAchievements, hateoasLearningActivities, hateoasLearningAEntitlements, hateoasLearningAssessments, hateoasIssuer, hateoasDiploma
        };
    }
}
