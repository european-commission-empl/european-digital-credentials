...
public class CredentialPublicResource {
    ...
    @ApiOperation(value = "Seal a credential in JSON format and download resulting file. Requires a configured local certificate.")
    @PostMapping(value = IssuerEndpoint.V1.CREDENTIALS_SEAL,
            consumes = MediaType.MULTIPART_FORM_DATA_VALUE,
            produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<String> sealAndDownloadCredential(@ApiParam(required = true, value = "The JSON file containig the credential") @RequestPart(Parameter.FILE) MultipartFile file,
                                                            @ApiParam(required = true, value = "The password for the local certificate ") @RequestParam(value = Parameter.PASSWORD) String password,
                                                            @ApiParam(required = false, value = "indicates that the credential should be signed on behalf of another organization") @RequestParam(value = Parameter.SIGN_ON_BEHALF, required = false, defaultValue = "false") boolean signOnBehalf) {
        PublicSealingDTO publicSealingDTO = new PublicSealingDTO(file, password, signOnBehalf);
        return new ResponseEntity<>(new String(this.getCredentialPublicService().doLocalSignAndDownloadCredential(publicSealingDTO), StandardCharsets.UTF_8),
                prepareHttpHeadersForCredentialDownload(file.getName(), MediaType.APPLICATION_JSON_VALUE),
                HttpStatus.OK);
