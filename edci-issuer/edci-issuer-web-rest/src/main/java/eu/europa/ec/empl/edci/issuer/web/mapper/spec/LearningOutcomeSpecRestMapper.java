...
public interface LearningOutcomeSpecRestMapper extends IRestMapper<LearningOutcomeSpecDAO, LearningOutcomeSpecView, LearningOutcomeSpecLiteView> {
    ...
    @Override
    default String getDisplayName(LearningOutcomeSpecDAO dao, String locale) {
        Optional<ContentDTDAO> titleContentDAO = dao.getName().getLocalizedContent(locale);
        String name = titleContentDAO.isPresent() ? titleContentDAO.get().getContent() : dao.getName().getStringContent(dao.getDefaultLanguage());
        return dao.getLabel() != null && !dao.getLabel().isEmpty() ?
                name.concat(EDCIConstants.StringPool.STRING_SPACE)
                        .concat(EDCIConstants.StringPool.STRING_HYPHEN)
                        .concat(EDCIConstants.StringPool.STRING_SPACE)
                        .concat(dao.getLabel()) : name;
    }
