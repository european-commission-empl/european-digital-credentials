...
public class EntitlementResource implements CrudResource {
    ...
    public ResponseEntity<PagedResources<EntitlementSpecLiteView>> listHasEntPart(
        ...
        return generateListResponse(entitlementDAO.getHasPart(), "/specs", entitlementSpecRestMapper);
    ...
    public ResponseEntity<Resource<OrganizationSpecView>> getValidWith(
        ...
        OrganizationSpecDAO orgSpec = null;
        ...
        if (entitlementDAO.getSpecifiedBy() != null && entitlementDAO.getSpecifiedBy().getLimitOrganization() != null
            ...
            orgSpec = entitlementDAO.getSpecifiedBy().getLimitOrganization().iterator().next();
        }

        return generateOkResponse(orgSpec, organizationSpecRestMapper, generateEntitlementHateoas(entitlementDAO));
    }
    ...
    @PostMapping(value = Endpoint.V1.SPECS + Parameter.Path.OID + Endpoint.V1.ENT_VALID_WITH,
            ...
            produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public ResponseEntity<Resource<OrganizationSpecView>> setValidWith(
            @ApiParam(required = true, value = "The Organization oid") @PathVariable(Parameter.OID) Long oid,
            @RequestBody SubresourcesOids oids,
            @ApiParam(value = "locale") @RequestParam(value = Parameter.LOCALE, required = false) String locale) throws Exception {
        ...
        return generateOkResponse(entitlementDAO.getSpecifiedBy().getLimitOrganization().iterator().next(), organizationSpecRestMapper, generateEntitlementHateoas(entitlementDAO));
