...
public class EntitlementResource implements CrudResource {
    ...
    public ResponseEntity<Resource<EntitlementSpecView>> createEntitlement(@RequestBody @Valid EntitlementSpecView entitlementView,
        ...
        EntitlementSpecDAO entitlementCreatedDAO = entitlementService.save(entitlementDAO,
                () -> {
                    ...
                    entitlementDAO.getSpecifiedBy().setLimitOrganisation(organizationSpecService.retrieveEntities(false, entitlementView.getRelValidWith().getOid()));
                    entitlementDAO.setAwardedBy(entitlementDAO.getAwardedBy() == null ? new AwardingProcessDCDAO() : entitlementDAO.getAwardedBy());
                    entitlementDAO.getAwardedBy().setAwardingBody(organizationSpecService.retrieveEntities(false, entitlementView.getRelAwardingBody().getOid()));
                }
        );
    ...
    @GetMapping(value = IssuerEndpoint.V1.SPECS + Parameter.Path.OID + IssuerEndpoint.V1.ENT_AWARDING_BODY,
            produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public ResponseEntity<PagedResources<OrganizationSpecLiteView>> listAwardingBodiesEnt(
            @ApiParam(required = true, value = "The Entitlement oid") @PathVariable(Parameter.OID) Long oid,
            @ApiParam(value = "locale") @RequestParam(value = Parameter.LOCALE, required = false) String locale
    ) throws Exception {
        ...
        if (entitlementDAO.getAwardedBy() == null) {
            entitlementDAO.setAwardedBy(new AwardingProcessDCDAO());
        }

        return generateListResponse(entitlementDAO.getAwardedBy().getAwardingBody(), "/specs", organizationSpecRestMapper, LocaleContextHolder.getLocale().toString());

    }

    @ApiOperation(value = "Link an existing subresource")
    @PostMapping(value = IssuerEndpoint.V1.SPECS + Parameter.Path.OID + IssuerEndpoint.V1.ENT_AWARDING_BODY,
            consumes = MediaType.APPLICATION_JSON_VALUE,
            produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public ResponseEntity<PagedResources<OrganizationSpecLiteView>> setAwardingBodiesEnt(
            @ApiParam(required = true, value = "The Entitlement oid") @PathVariable(Parameter.OID) Long oid,
            @RequestBody SubresourcesOids oids,
            @ApiParam(value = "locale") @RequestParam(value = Parameter.LOCALE, required = false) String locale) throws Exception {
        ...
        entitlementDAO.getAwardedBy().setAwardingBody(entities);
    ...
    public ResponseEntity<PagedResources<OrganizationSpecLiteView>> getValidWith(
        ...
        if (entitlementDAO.getSpecifiedBy() != null && entitlementDAO.getSpecifiedBy().getLimitOrganisation() != null) {
            orgSpecs = entitlementDAO.getSpecifiedBy().getLimitOrganisation();
        }
    ...
    public ResponseEntity<PagedResources<OrganizationSpecLiteView>> setValidWith(
        ...
        entitlementDAO.getSpecifiedBy().setLimitOrganisation(entities);

        entitlementService.save(entitlementDAO);

        return generateListResponse(entitlementDAO.getSpecifiedBy().getLimitOrganisation(), "/specs", organizationSpecRestMapper, LocaleContextHolder.getLocale().toString());
    ...
    public ResponseEntity deleteProvenBy(
        ...
        if (entitlementDAO.getSpecifiedBy() != null && entitlementDAO.getSpecifiedBy().getLimitOrganisation() != null
                && entitlementDAO.getSpecifiedBy().getLimitOrganisation().size() > 0) {
            entitlementDAO.getSpecifiedBy().getLimitOrganisation().clear();
            entitlementService.save(entitlementDAO);
        }
