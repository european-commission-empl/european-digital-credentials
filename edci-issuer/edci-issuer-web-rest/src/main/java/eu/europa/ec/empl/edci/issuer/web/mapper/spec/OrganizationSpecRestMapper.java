...
public interface OrganizationSpecRestMapper extends IRestMapper<OrganizationSpecDAO, OrganizationSpecView, OrganizationSpecLiteView> {
    ...
    default String getDisplayName(OrganizationSpecDAO dao, String locale) {
        if (dao.getLegalName() == null) {
            return "";
        }

        Optional<ContentDTDAO> titleContentDAO = dao.getLegalName().getLocalizedContent(locale);

        if (titleContentDAO == null) {
            return "";
        }

        String title = titleContentDAO.isPresent() ? titleContentDAO.get().getContent() : dao.getLegalName().getStringContent(dao.getDefaultLanguage());
