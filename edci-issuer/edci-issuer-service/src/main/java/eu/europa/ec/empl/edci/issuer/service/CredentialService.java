...
public class CredentialService {
    ...
    private final static String WALLET_ADDRESS_ERROR = " for wallet address ";
    private final static String WALLET_EMAIL_ERROR = " for email ";
     ...
     * "credential" : { %EuropeanDigitalCredential }
     * "deliveryDetails" : {
     * "deliveryAddress" : ["123456abc", "example@mail.com"]
     * }
     * }
     *
     * @param files the credential files
    ...
    public List<EuropeanDigitalCredentialUploadResultDTO> obtainCredentials(MultipartFile[] files, int maximumConsecutiveFailures, int maximumTotalFailures) {
        ...
        for (MultipartFile file : files) {
            ...
            try {
                edcur = this.getJsonUtil().unMarshall(file.getBytes(), EuropeanDigitalCredentialUploadResultDTO.class);
                int limit = this.getIssuerConfigService().getInteger(DataModelConstants.Properties.DELIVERY_ADDRESS_LIMIT, 2);
                if (edcur.getDeliveryDetails() == null || edcur.getDeliveryDetails().getDeliveryAddress().isEmpty()
                        || edcur.getDeliveryDetails().getDeliveryAddress().size() > limit) {
                    edcur.setErrorAddress(true);
                }
    ...
    public CredentialFileDTO uploadParsedCredentials(List<EuropeanDigitalCredentialUploadDTO> europeanDigitalCredentialUploadDTOS, String folder, String locale) {
        ...
        this.doProcessCredentials(europeanDigitalCredentialUploadDTOS, folder);
    ...
    public CredentialFileDTO issueCredentials(List<EuropeanDigitalCredentialUploadDTO> credentials, String locale) {
        ...
        this.doProcessCredentials(credentials, fileUtil.getCredentialPrivateFolderName(sessionId));
     ...
     *
     * @param europeanDigitalCredentialUploadDTOS the credentials to be processed
     */
    private void doProcessCredentials(List<EuropeanDigitalCredentialUploadDTO> europeanDigitalCredentialUploadDTOS, String folder) {

        List<EuropeanDigitalCredentialDTO> credentials = europeanDigitalCredentialUploadDTOS.stream().map(upload -> upload.getCredential()).collect(Collectors.toList());

        //process credentials with consumers
        List<ConsumerContext> credentialContexts = europeanDigitalCredentialUploadDTOS.stream().map(ConsumerContext::new).collect(Collectors.toList());
        this.getAndExecuteCredentialConsumers(credentialContexts);
        for (EuropeanDigitalCredentialDTO credentialDTO : credentials) {
     ...
     */
    public ResponseEntity<byte[]> downloadZipFile(List<String> uuids, Map<String, DeliveryDetailsDTO> deliveryDetailsMap) {
        String sessionId = getSessionId();
        String zipFile = "credentials_" + new SimpleDateFormat("yyyyMMdd_hhmm").format(new Date()) + ".zip";
        Map<String, String> credentialList = uuids.stream().collect(Collectors.toMap(uuid -> uuid, uuid -> this.getFileUtil().getFileName(URI.create(uuid))));

        String sessionFolder = this.getFileUtil().getCredentialPrivateFolderName(sessionId);
        byte[] zipBytes = null;
        try {
            zipUtil.addFilesToZIP(sessionFolder, zipFile, credentialList, deliveryDetailsMap);
        ...
        for (String uuid : uuidList) {
            this.getEdciExecutorService().submitTask(executorName, () -> {
                ...
                try {
                    ...
                    //Throws Exception
                    credSignatureBytes.setValid(false);
                    credSignatureBytes.setErrorMessage(e.getCause() != null ? e.getCause().getMessage() : this.getEdciMessageService().getMessage(e.getMessageKey(), e.getMessageArgs()));
    ...
    private void sendWalletCredentials(List<CredentialDTO> credentialDTOs) {
        for (CredentialDTO credentialDTO : credentialDTOs) {
            ...
            if (!this.getValidator().isEmpty(credentialDTO.getWalletAddress()) || !this.getValidator().isEmpty(credentialDTO.getEmail())) {
                ...
                parameters.put("sendEmail", String.valueOf(sendByEmail));
                boolean atLeastOneSent = false;

                if (!this.getValidator().isEmpty(credentialDTO.getWalletAddress())) {
                    for (String address : credentialDTO.getWalletAddress()) {
                        try {
                            parameters.put(Parameter.WALLET_ADDRESS, address);
                            this.getWalletResourceUtil().doWalletPostRequest(walletApiUrl, multipartFile, EDCIParameter.WALLET_ADD_CREDENTIAL, CredentialUploadResponseDTO.class, MediaType.APPLICATION_JSON, false, parameters);
                            atLeastOneSent = true;
                        } catch (NullPointerException e) {
                            receivedErrors.add(this.getEdciMessageService().getMessage(new EDCIException().getMessageKey()) + WALLET_ADDRESS_ERROR + address);
                        } catch (EDCIException e) {
                            //Error Recieved from wallet, (already translated) must be passed to VIEW.
                            if(e.getMessage() != null && !e.getMessage().contains(address)) {
                                receivedErrors.add(e.getMessage() + WALLET_ADDRESS_ERROR + address);
                            } else {
                            ...
                            logger.error(e);
                            receivedErrors.add(this.getEdciMessageService().getMessage(EDCIIssuerMessageKeys.GLOBAL_INTERNAL_ERROR)  + WALLET_ADDRESS_ERROR + address);
                        }
                    }
                ...
                if (!this.getValidator().isEmpty(credentialDTO.getEmail())) {
                    for (String address : credentialDTO.getEmail()) {
                        try {
                            if (!this.getValidator().isEmpty(address) && !EmailValidator.getInstance().isValid(address.replaceFirst("mailto:", ""))) {
                                throw new EDCIException(this.getEdciMessageService().getMessage(EDCIIssuerMessageKeys.ERROR_INVALID_EMAIL, address.replaceFirst("mailto:", "")));
                            }
                            parameters.put(Parameter.WALLET_USER_EMAIL, address);
                            this.getWalletResourceUtil().doWalletPostRequest(walletApiUrl, multipartFile, EDCIParameter.WALLET_ADD_CREDENTIAL, CredentialUploadResponseDTO.class, MediaType.APPLICATION_JSON, false, parameters);
                            atLeastOneSent = true;
                        } catch (NullPointerException e) {
                            receivedErrors.add(this.getEdciMessageService().getMessage(new EDCIException().getMessageKey())  + WALLET_EMAIL_ERROR + address);
                        } catch (EDCIException e) {
                            //Error Recieved from wallet, (already translated) must be passed to VIEW.
                            if(e.getMessage() != null && !e.getMessage().contains(address)) {
                                receivedErrors.add(e.getMessage() + WALLET_EMAIL_ERROR + address);
                            } else {
                                receivedErrors.add(e.getMessage());
                            }
                        } catch (EDCIRestException e) {
                            ...
                            logger.error(e);
                            receivedErrors.add(this.getEdciMessageService().getMessage(EDCIIssuerMessageKeys.GLOBAL_INTERNAL_ERROR)  + WALLET_EMAIL_ERROR + address);
                        }
                    }
                }

                credentialDTO.setReceived(atLeastOneSent);
                credentialDTO.setSent(atLeastOneSent);
            } else {
        ...
        for (CredentialDTO credentialDTO : credentialDTOS) {
            for (String address : credentialDTO.getEmail()) {
                ...
                if (!this.getValidator().isEmpty(toEmail) && !EmailValidator.getInstance().isValid(toEmail.replaceFirst("mailto:", ""))) {
                    credentialDTO.getSendErrors().add(this.getEdciMessageService().getMessage(EDCIIssuerMessageKeys.ERROR_INVALID_EMAIL, toEmail));
