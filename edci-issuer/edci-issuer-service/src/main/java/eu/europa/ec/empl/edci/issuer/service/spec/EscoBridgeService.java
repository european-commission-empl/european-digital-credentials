...
public class EscoBridgeService {
    ...
    private static final Logger logger = Logger.getLogger(EscoBridgeService.class);

    public enum EscoList {

        OCCUPATION("http://data.europa.eu/esco/model#Occupation", "occupation"),
        SKILL("http://data.europa.eu/esco/model#Skill", "skill");
    ...
    public Page<CodeDTDAO> searchEscoElements(
        ...
        Page pageResult = new PageImpl<CodeDTDAO>(
                results.getEmbedded().getResults().stream().map(skill -> controlledListsMapper.toCodeDAOESCO(skill, retrieveLangs)).collect(Collectors.toList()),
    ...
    public List<CodeDTDAO> searchEscoElements(
        ...
        for (String uri : uris) {
            ...
            skosList.add(controlledListsMapper.toCodeDAOESCO(result, retrieveLangs));
