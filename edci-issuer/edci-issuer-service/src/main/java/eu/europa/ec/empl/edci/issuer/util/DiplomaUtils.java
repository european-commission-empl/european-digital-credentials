...
public class DiplomaUtils {
    ...
    public void informDiplomaFromTemplate(EuropeanDigitalCredentialDTO credential,
                                          String template, Set<LabelDTDAO> labels, MediaObjectDTDAO background, boolean isDiplomaSupplement) {
        ...
        for (String currentLang : availableLangs.keySet()) {
            ...
            String diplomaHTML = getThymeleafUtil().processTemplate(template, context, labelsProp);
    ...
    protected String buildImageBase64(MediaObjectDTDAO mediaObjectElement) {
        String value = null;
        try {
            String base64Content = new String(Base64.getEncoder().encode(mediaObjectElement.getContent()), StandardCharsets.UTF_8); //TODO review
            String extension = controlledListsUtil.getMimeType(mediaObjectElement.getContentType().getUri());
