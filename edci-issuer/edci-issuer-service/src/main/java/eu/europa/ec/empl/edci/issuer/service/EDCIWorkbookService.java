...
public class EDCIWorkbookService implements IWorkBookService {
    ...
    private static final Logger logger = LogManager.getLogger(EDCIWorkbookService.class);
    ...
    public List<EuropassCredentialDTO> parseCredentialData(Workbook workbook) {
        ...
        credentialService.preProcessCredentials(europassCredentialDTOS.stream().map(ec -> new ConsumerContext(ec)).collect(Collectors.toList()));
        /*
        ...
        credentialService.postProcessCredentials(processedCredentials.stream().map(ec -> new ConsumerContext(ec)).collect(Collectors.toList()));
        ...
        for (EuropassCredentialDTO europassCredentialDTO : europassCredentialDTOS) {
            try {
                ...
                for (GradeObject holderGrade : holderGrades) {
                    ...
                    List<S> holderGradeableItems = reflectiveUtil.getInnerObjectsOfType(gradeableClass, holder);
