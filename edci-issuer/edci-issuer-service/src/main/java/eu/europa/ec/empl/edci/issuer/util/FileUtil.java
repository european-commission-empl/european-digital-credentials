...
public class FileUtil {
   ...
   /* public EuropassCredentialDTO getEuropassCredentialFromFile(CredentialDTO credentialDTO) throws IOException, JAXBException {
    ...
    public String getCredentialPublicFolderName(String batchFolder) {
        ...
        if (batchFolder != null) {
            folderName = folderName.concat(EDCIConstants.StringPool.STRING_SLASH) + batchFolder;
        }
    ...
    public String getCredentialFileAbsolutePath(CredentialDTO credentialDTO, @Nullable String fileBatch) {
        return fileBatch != null ?
                this.getCredentialPublicFolderName(fileBatch).concat(this.getFileName(URI.create(credentialDTO.getUuid())))
                : getCredentialFileAbsolutePath(credentialDTO.getUuid());
    }

    public String getCredentialFileAbsolutePath(String credId) {
        String hashPath = RequestContextHolder.currentRequestAttributes().getSessionId();
        return getCredentialPrivateFolderName(hashPath).concat(getFileName(URI.create(credId)));
    }
