...
import eu.europa.ec.empl.edci.issuer.common.model.customization.CustomizedFieldDTO;
import eu.europa.ec.empl.edci.issuer.common.model.customization.CustomizedRecipientDTO;
...
public interface CredentialMapper {
    ...
    @Mappings({
            ...
            @Mapping(source = "xmlCredentialDTO.credentialSubject", qualifiedByName = "getStudentName", target = "studentName"),
    ...
    default String getStudentName(PersonDTO credentialSubject, @Context String locale) {
        return credentialSubject.getFullName() != null ? credentialSubject.getFullName().getStringContent(locale) :
                credentialSubject.getGivenNames().getStringContent(locale) + " " + credentialSubject.getFamilyName().getStringContent(locale);
    }
    ...
    EuropassCredentialDTO toDTO(EuropassCredentialSpecDAO credential);

//    default EuropassCredentialDTO fromCustomToDTO(EuropassCredentialSpecDAO credential, CustomizedRecipientsDTO recipients, @Context String locale){
//
//        EuropassCredentialDTO credentialDTO = toDTO(credential);
//        getCustomizableEntityIdentifierField
//
//        recipients.getRecipients().iterator().next().getFields();
//        for(CustomizedRecipientDTO recipìent: recipients.getRecipients()) {
//            for(CustomizedFieldDTO field: recipìent.getFields()) {
//                List<String> pathList = Arrays.stream(field.getFieldPathIdentifier().split("\\.")).collect(Collectors.toList());
//
//                for(String seccion: pathList) {
