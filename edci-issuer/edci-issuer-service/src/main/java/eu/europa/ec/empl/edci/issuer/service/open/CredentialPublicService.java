...
public class CredentialPublicService {
    ...
    public ApiErrorMessage doBatchSealAndSendCredentials(PublicBatchSealingDTO publicBatchSealingDTO) {
        ...
        for (int i = 0; i < publicBatchSealingDTO.getFiles().length; i++) {
            ...
            try {
                /*EuropeanDigitalCredentialDTO europeanDigitalCredentialDTO = this.readAndCheckCredential(publicBatchSealingDTO.getFiles()[i]);
                if (europeanDigitalCredentialDTO == null) throw new EDCIBadRequestException();*/
    ...
    public byte[] doLocalSignAndDownloadCredential(PublicSealingDTO publicSealingDTO) {
        //read cred
        /*EuropeanDigitalCredentialDTO europeanDigitalCredentialDTO = this.readAndCheckCredential(publicSealingDTO.getFile());
        return this.doLocalSignCredential(publicSealingDTO, europeanDigitalCredentialDTO.getClass());*/
        byte[] bytes = null;
        return bytes;
    }
    ...
    public PublicSealAndSendDTO doLocalSignAndSendCredential(PublicSealingDTO publicSealingDTO) {
        ...
        /*String email = this.getEdciCredentialModelUtil().getSubjectFirstEmail(europeanDigitalCredentialDTO);
        String walletAddress = this.getEdciCredentialModelUtil().getSubjectFirstWalletAddress(europeanDigitalCredentialDTO);
        ...
        if ((email == null && walletAddress == null) || (!sendTemporal && walletAddress == null)) {
            ...
            byte[] credentialBytes = this.doLocalSignCredential(publicSealingDTO, europeanDigitalCredentialDTO.getClass());
    ...
    private byte[] doLocalSignCredential(PublicSealingDTO publicSealingDTO, Class type) {
        ...
        //Get Local Cert path
        String certPath = this.getIssuerConfigService().getString(EDCIConfig.DSS.CERT_PATH);
//        String xPath = ESealConstants.EUROPASS_CREDENTIAL_XPATH_LOCATION;
...
//                xPath = DSSConstants.VERIFIABLE_PRESENTATION_XPATH_LOCATION;
        ...
        try {
            this.getCertificateUtils().overwriteCertificateFields(filePath, certInfo, filePath);
        ...
        //sign XML Credential
        DSSDocument dssDocument = this.getDssedciSignService().signDocument(filePath, certPath, publicSealingDTO.getCertPassword());
    ...
    /*private CredentialHolderDTO readAndCheckCredential(MultipartFile file) {
        ...
        try {
            ...
            //TODO call DSS validation service isSigned
            if (this.getESealValidationService().isSigned(unsignedBytes)) {
    ...
    private File createTemporalFile(String fileName, String folderPath, MultipartFile file) {
        ...
        try {
            ...
            EDCIBadRequestException edciException = new EDCIBadRequestException(HttpStatus.BAD_REQUEST,
                    ...
                    EDCIIssuerMessageKeys.CREDENTIAL_FILE_NOT_FOUND,
                    filePath);
            logger.error("Could not create temporal credential %s [%s]", () -> folderPath.concat(fileName), () -> e.getMessage(), () -> e);
            throw edciException;
        }
    }
