...
public class SPARQLITest extends AbstractIntegrationBaseTest {
    ...
    public void searchConcepts_shouldReturnValues_whenCalled() throws Exception {
        ...
        /*Mockito.when(iConfigService.getString(DataModelConstants.CONFIG_PUBLICATIONS_RDF_SPARQL_ENDPOINT)).thenReturn("http://publications.europa.eu/webapi/rdf/sparql");
        List<RDFConcept> code = rdfSparqlBridgeService.searchRDFConcepts(ControlledList.HUMAN_SEX.getUrl(), "ma", "en", 0, 1000, Arrays.asList("en", "es", "it"));
        Assert.assertNotEquals(0, code.size());
        List<RDFConcept> extRes = rdfSparqlBridgeService.searchRDFConcepts(ControlledList.CREDENTIAL_TYPE.getUrl(), "", "en", 0, 1000, Arrays.asList("en", "es", "it"));
